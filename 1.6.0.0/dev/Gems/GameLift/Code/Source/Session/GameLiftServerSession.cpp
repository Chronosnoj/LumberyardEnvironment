...
namespace GridMate
{
    ...
    public:
        ...
        void OnReplicaDeactivate(const ReplicaContext& rc) override
        {
            if (IsMaster() && !IsLocal())
            {
                Aws::GameLift::GenericOutcome outcome = Aws::GameLift::Server::RemovePlayerSession(GetId().ToString().c_str());
                if (!outcome.IsSuccess())
                {
                    AZ_TracePrintf("GameLift", "Failed to disconnect GameLift player:'%s' with id=%s\n", outcome.GetError().GetErrorName().c_str(), GetId().ToString().c_str());
                }
            }
