...
class CProceduralMaterialFlowNodeRenderASync : public CProceduralMaterialFlowNodeRenderBase
{
public:
	CProceduralMaterialFlowNodeRenderASync(SActivationInfo* pActInfo)
		: CProceduralMaterialFlowNodeRenderBase()
		, m_ASyncUID(0)
	{
	}
	...
	IFlowNodePtr Clone(SActivationInfo *pActInfo) override
	{
		return new CProceduralMaterialFlowNodeRenderASync(pActInfo);
	}
...
protected:
	...
	{
		static const SOutputPortConfig out_config[] = {
			OutputPortConfig_Void("RenderBegin", _HELP("Called when Rendering has been invoked")),
			OutputPortConfig_Void("RenderComplete", _HELP("Called when Rendering has completed")),
			{ 0 }
		};
		return out_config;
	}
	...
	virtual void DoRender(SActivationInfo* pActInfo) override
	{
		EBUS_EVENT_RESULT(m_ASyncUID, SubstanceRequestBus, RenderASync);

		ActivateOutput(pActInfo, eO_RenderBegin, true);

		//if no commands were issued, activate output immediately
		if (m_ASyncUID == INVALID_PROCEDURALMATERIALRENDERUID)
		{
			ActivateOutput(pActInfo, eO_RenderComplete, true);
		}
		else
	...
	virtual void DoUpdate(SActivationInfo* pActInfo) override
	{
		//query substance system for completion of render commands
		bool renderCompleted = false;
		EBUS_EVENT_RESULT(renderCompleted, SubstanceRequestBus, HasRenderCompleted, m_ASyncUID);
		if (renderCompleted)
		{
			ActivateOutput(pActInfo, eO_RenderComplete, true);

			m_ASyncUID = INVALID_PROCEDURALMATERIALRENDERUID;
...
private:
	...
	ProceduralMaterialRenderUID m_ASyncUID;
};
REGISTER_FLOW_NODE("ProceduralMaterial:RenderASync", CProceduralMaterialFlowNodeRenderASync)
#endif // USE_SUBSTANCE
