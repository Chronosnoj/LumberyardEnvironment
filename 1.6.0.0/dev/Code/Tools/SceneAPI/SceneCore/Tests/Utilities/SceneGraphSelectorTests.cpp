    ...
    namespace SceneAPI
    {
        namespace Utilities
        {
            ...
            public:
                ...
                static bool IsValidTestNodeType(const Containers::SceneGraph& graph, Containers::SceneGraph::NodeIndex& index)
                {
                    return true;
                }

                static bool IsVaidTestNodeTypeSomeInvalid(const Containers::SceneGraph& graph, Containers::SceneGraph::NodeIndex& index)
                {
                    if (strcmp(graph.GetNodeName(index).c_str(), "") == 0 || strcmp(graph.GetNodeName(index).c_str(), "A.D") == 0)
                    {
                        return false;
                    }
                    else
            ...
            TEST_F(SceneGraphSelectorTest, GenerateTargetNodes_OnlySelectedRootNodeSomeInvalidNodes_NodeAandABandACandADE)
            {
                ...
                AZStd::vector<AZStd::string> unselectedNodes;
                CreateMeshGroup(selectedNodes, unselectedNodes);
                auto targetNodes = SceneGraphSelector::GenerateTargetNodes(m_graph, m_testNodeSelectionList, IsVaidTestNodeTypeSomeInvalid);
