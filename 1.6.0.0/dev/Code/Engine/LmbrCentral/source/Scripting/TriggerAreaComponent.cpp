...
namespace LmbrCentral
{
    AZ_SCRIPTABLE_EBUS(
        ...
        TriggerAreaRequestsBus,
        "{940536D8-62C1-404D-9964-039369F31D55}",
        "{C452AD6C-69C3-479F-9BB1-0CA5A609C4E1}",
        AZ_SCRIPTABLE_EBUS_EVENT(AddRequiredTag, const Tag&)
        AZ_SCRIPTABLE_EBUS_EVENT(RemoveRequiredTag, const Tag&)
        AZ_SCRIPTABLE_EBUS_EVENT(AddExcludedTag, const Tag&)
        AZ_SCRIPTABLE_EBUS_EVENT(RemoveExcludedTag, const Tag&)
        )

    AZ_SCRIPTABLE_EBUS(
        TriggerAreaNotificationBus,
        TriggerAreaNotificationBus,
        "{CA8AB650-F84D-4E05-90C3-98F9C8A47694}",
        "{D3C88952-AA9A-48AA-8C2C-9011020A6AEA}",
        AZ_SCRIPTABLE_EBUS_EVENT(OnTriggerAreaEntered, AZ::EntityId)
        AZ_SCRIPTABLE_EBUS_EVENT(OnTriggerAreaExited, AZ::EntityId)
        )

    AZ_SCRIPTABLE_EBUS(
        TriggerAreaEntityNotificationBus,
        TriggerAreaEntityNotificationBus,
        "{5861D736-5975-48BD-8734-CC40ADDF8D22}",
        "{4CCF51B5-DF0A-436F-97BD-9EF208D17CC8}",
        AZ_SCRIPTABLE_EBUS_EVENT(OnEntityEnteredTriggerArea, AZ::EntityId)
        AZ_SCRIPTABLE_EBUS_EVENT(OnEntityExitedTriggerArea, AZ::EntityId)
        )
    ...
    public:        
        ...
        GridMate::ReplicaChunkBase* CreateFromStream(GridMate::UnmarshalContext& context) override
        {
            ...
            {
                triggerAreaChunk->m_initialEntitiesInArea.push_back(AZ::EntityId(entityId));
            }
    ...
    void TriggerAreaComponent::Reflect(AZ::ReflectContext* context)
    {
        ...
        if (script)
        {
            ScriptableEBus_TriggerAreaNotificationBus::Reflect(script);
            ScriptableEBus_TriggerAreaEntityNotificationBus::Reflect(script);
            ScriptableEBus_TriggerAreaRequestsBus::Reflect(script);
        }
    ...
    // OnTransformChanged
    //=========================================================================
    void TriggerAreaComponent::OnTransformChanged(const AZ::Transform& /*local*/, const AZ::Transform& world)
    {
    ...
    void TriggerAreaComponent::CreateProximityTrigger()
    {
        ...
        if (m_proximityTrigger == nullptr)
        {
            ...
            OnTransformChanged(AZ::Transform::CreateIdentity(), entityTransform);
        }
    }
    ...
    void TriggerAreaComponent::OnTriggerEnter(AZ::EntityId entityId)
    {
        ...
        if (!IsNetworkControlled())
        {
            ...
            {
                static_cast<TriggerAreaReplicaChunk*>(m_replicaChunk.get())->OnAreaEnter(entityId);
            }
        }
    }
    ...
    void TriggerAreaComponent::OnTriggerExit(AZ::EntityId entityId)
    {
        ...
        if (!IsNetworkControlled())
        {
            ...
            if (result && m_replicaChunk && m_replicaChunk->IsMaster())
            {
                static_cast<TriggerAreaReplicaChunk*>(m_replicaChunk.get())->OnAreaExit(entityId);
            }
        }
    }
    ...
    bool TriggerAreaComponent::IsPlayer(AZ::EntityId entityId)
    {
        ...
        {
            const /*Cry*/ EntityId id = static_cast<EntityId>(entityId);
    ...
    void TriggerAreaComponent::OnShapeChanged()
    {
        AZ_Warning("[TriggerAreaComponent]", false, "Trigger area component does not currently support dynamic changes to trigger shape ...Entity %s [%llu]..."
