...
EXEMPT_USE_MODULES = ['rapidjson', 'rapidxml', 'gmock', 'AZCodeGenerator', 'libeay32', 'ssleay32']
EXEMPT_SPEC_MODULES = ['MetricsPlugin','ProjectConfigurator', 'DXOrbisShaderCompiler']
EXEMPT_LIBRARY_MODULES = lumberyard_sdks.BuildNativeSDKLibraryList() + lumberyard_sdks.BuildGameScaleLibraryList()
EXEMPT_GEMS_USE_MODULES = set()
...
def _is_module_exempt(ctx, target_name):
    ...
    # Ignore any gems modules (lazy load)
    if len(EXEMPT_GEMS_USE_MODULES)==0:
        [EXEMPT_GEMS_USE_MODULES.add(gems_output_name) for gems_output_name in ctx.GetGemsModuleNames()]
    if target_name in EXEMPT_GEMS_USE_MODULES:
        return True
    ...
    def _read_module_def_file(ctx, target_name, parent_module, parent_spec):
        ...
        if module_def is None:
            if parent_module is not None:
                # Missing module explicitly defined as a 'use' in a wscript for a module
                if not lumberyard_sdks.convert_dual_task_name_to_lib_name(target_name) in EXEMPT_LIBRARY_MODULES:
                    Logs.warn('[WARN] Invalid module {} defined as a use dependency in wscript for module \'{}\''.format(target_name,parent_module))
            elif parent_spec is not None:
                # Only show the warning for modules that are on exempt module spec list
                if not target_name in EXEMPT_SPEC_MODULES:
                    # Missing module explicitly declared in one of the specs
