...
void usage(AzTest::Platform& platform)
{
...
int main(int argc, char** argv)
{
    ...
    AzTest::Platform& platform = AzTest::GetPlatform();
    ...
    AzTest::Utils::removeParameters(argc, argv, 1, 2);
    ...
    for (int i = 0; i < argc; i++)
    {
        if (strcmp(argv[i], "--wait-for-debugger") == 0)
        {
            ...
            AzTest::Utils::removeParameters(argc, argv, i, i);
            i--;
        }
        if (strcmp(argv[i], "--integ") == 0)
        {
            isInteg = true;
            AzTest::Utils::removeParameters(argc, argv, i, i);
            i--;
    ...
    std::shared_ptr<AzTest::IModuleHandle> bootstrap;
    if (isInteg)
    {
    ...
    int result = 0;
    {
        std::shared_ptr<AzTest::IModuleHandle> module = platform.GetModule(lib);
        if (module->IsValid())
        {
