    ...
    namespace SceneAPI
    {
        ...
        namespace Containers
        {
            ...
            public:
                ...
                using NameStorageType = std::string;
                ...
                SCENE_CORE_API NodeIndex Find(const std::string& path) const;
                SCENE_CORE_API NodeIndex Find(NodeIndex root, const char* relativePath) const;
                SCENE_CORE_API NodeIndex Find(NodeIndex root, const std::string& relativePath) const;
                ...
                SCENE_CORE_API const std::string& GetNodeName(NodeIndex node) const;
                ...
                // Checks if the given name can be used as a valid name for a node. This only checks the name validity, not if it's
                //      already in use. Use Find(...) to check if a name is already in use.
                SCENE_CORE_API static bool IsValidName(const std::string& name);

                // Names returned by this api contain the fully specified name for a name to guaranteed uniqueness. This function
                //      strips the leading path and returns the name under which the node was originally registered.
                SCENE_CORE_API static std::string GetShortName(const char* name);
                // Names returned by this api contain the fully specified name for a name to guaranteed uniqueness. This function
                //      strips the leading path and returns the name under which the node was originally registered.
                SCENE_CORE_API static std::string GetShortName(const std::string& name);
            ...
            private:
                ...
                NameLookup::const_iterator FindNameLookupIterator(const std::string& name) const;
                NameLookup::const_iterator FindNameLookupIterator(StringHash hash, const std::string& name) const;

                std::string CombineName(const std::string& path, const char* name) const;
                void AddDefaultRoot();
