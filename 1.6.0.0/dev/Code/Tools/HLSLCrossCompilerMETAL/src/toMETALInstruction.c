    ...
    uint32_t typeFlag, Instruction* psNextInst)
{
    ...
    AddIndentation(psContext);
    AddAssignToDest(psContext, &psInst->asOperands[0], SVT_INT, destElemCount, &needsParenthesis);
...
static void AddMOVCBinaryOp(HLSLCrossCompilerContext* psContext, const Operand* pDest, const Operand* src0, Operand* src1, Operand* src2)
{
    ...
    */
    ...
    if (s0ElemCount == 1 || IsSwizzleReplicatedMETAL(src0))
    {
        ...
        {
            TranslateOperandMETAL(psContext, src1, SVTTypeToFlagMETAL(eDestType) | ElemCountToAutoExpandFlag(destElemCount));
        }
        else
        {
        ...
        {
            TranslateOperandMETAL(psContext, src2, SVTTypeToFlagMETAL(eDestType) | ElemCountToAutoExpandFlag(destElemCount));
        }
        else
        {
    ...
    int dest, int src0, int src1, int src2, uint32_t dataType)
{
    ...
    AddAssignToDest(psContext, &psInst->asOperands[dest], TypeFlagsToSVTTypeMETAL(dataType), dstSwizCount, &numParenthesis);
    ...
    bstring metal)
{
    ...
    AddIndentation(psContext);
    AddAssignToDest(psContext, &psInst->asOperands[0], TypeFlagsToSVTTypeMETAL(ResourceReturnTypeToFlag(psBinding->ui32ReturnType)), 4, &numParenthesis);
    ...
    uint32_t ui32Flags)
{
    ...
    if (ui32Flags & TEXSMP_FLAG_DEPTHCOMPARE)
    {
        ...
        AddIndentation(psContext);
        AddAssignToDest(psContext, &psInst->asOperands[0], dataType, GetNumSwizzleElementsMETAL(&psInst->asOperands[2]), &numParenthesis);
...
static ShaderVarType* LookupStructuredVarAtomic(HLSLCrossCompilerContext* psContext,
    Operand* psResource,
    Operand* psByteOffset,
    uint32_t ui32Component)
{
...
void TranslateAtomicMemOpMETAL(HLSLCrossCompilerContext* psContext, Instruction* psInst)
{
    ...
    if (previousValue)
    {
        ...
        AddAssignToDest(psContext, previousValue, SVT_UINT, 1, &numParenthesis);
    }
    ...
    else
    {
        ...
        if (psRes->eBindArea == UAVAREA_CBUFFER)
        {
            ...
            else if (psRes->eType == RTYPE_UAV_RWSTRUCTURED)
            {
                ...
                {
                    psVarType = LookupStructuredVarAtomic(psContext, dest, destAddr, 0);
                }
    ...
    else
    {
...

        {
            ...
            else if (psRes->eType == RTYPE_UAV_RWSTRUCTURED)
            {
                if (destAddr->eType == OPERAND_TYPE_IMMEDIATE32)
                {
                    psAtomicList->AtomicVars[psAtomicList->Filled] = LookupStructuredVarAtomic(psContext, dest, destAddr, 0);
                    psAtomicList->Filled++;
                }
            }
        }
...
void TranslateInstructionMETAL(HLSLCrossCompilerContext* psContext, Instruction* psInst, Instruction* psNextInst)
{
    ...
    case OPCODE_RSQ:
    {
        ...
        bcatcstr(metal, "//RSQ\n");
#endif
        CallHelper1(psContext, "rsqrt", psInst, 0, 1, 1);
        break;
    }
    case OPCODE_EXP:
    {
...
#endif
        CallHelper1(psContext, "rint", psInst, 0, 1, 1);
        break;
    }
    case OPCODE_FRC:
    {
    ...
    case OPCODE_GATHER4_C:
    {
        ...
        AddAssignToDest(psContext, &psInst->asOperands[0], SVT_FLOAT, GetNumSwizzleElementsMETAL(&psInst->asOperands[2]), &numParenthesis);
        bcatcstr(metal, "(");
    ...
    case OPCODE_BREAKC:
    {
        ...
        TranslateConditional(psContext, psInst, metal);
        break;
    }
    case OPCODE_CONTINUEC:
    {
    ...
    case OPCODE_SYNC:
    {
        ...
        {
            AddIndentation(psContext);
            bcatcstr(metal, "threadgroup_barrier(mem_flags::mem_none);\n");
        }
        if (ui32SyncFlags & SYNC_THREAD_GROUP_SHARED_MEMORY)
        {
            AddIndentation(psContext);
            bcatcstr(metal, "threadgroup_barrier(mem_flags::mem_threadgroup);\n");
        }
        if (ui32SyncFlags & (SYNC_UNORDERED_ACCESS_VIEW_MEMORY_GROUP | SYNC_UNORDERED_ACCESS_VIEW_MEMORY_GLOBAL))
        {
            AddIndentation(psContext);
            bcatcstr(metal, "threadgroup_barrier(mem_flags::mem_device_and_threadgroup);\n");
        }
        break;
    }
    case OPCODE_SWITCH:
    ...
    case OPCODE_LD_MS:
    {
        ...
        //if (psInst->bAddressOffset)
        //{
        //  TranslateTexelFetchOffset(psContext, psInst, psBinding, metal);
        //}
        //else
        //{
        TranslateTexelFetch(psContext, psInst, psBinding, metal);
        //}
        break;
    }
    case OPCODE_DISCARD:
    ...
    case OPCODE_STORE_UAV_TYPED:
    {
        ...
        if (psRes->eBindArea == UAVAREA_CBUFFER)
        {
            ...
            bcatcstr(metal, "]=");
            TranslateOperandWithMaskMETAL(psContext, &psInst->asOperands[2], ResourceReturnTypeToFlag(psRes->ui32ReturnType), OPERAND_4_COMPONENT_MASK_X);
            bcatcstr(metal, ";\n");
        }
        else if (psRes->eBindArea == UAVAREA_TEXTURE)
        {
            ...
            TranslateOperandWithMaskMETAL(psContext, &psInst->asOperands[2], ResourceReturnTypeToFlag(psRes->ui32ReturnType), OPERAND_4_COMPONENT_MASK_ALL);
...
#endif
        AddIndentation(psContext);
        AddAssignToDest(psContext, &psInst->asOperands[0], SVT_INT, GetNumSwizzleElementsMETAL(&psInst->asOperands[1]), &numParenthesis);
