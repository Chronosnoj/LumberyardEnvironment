...
static bool GetFromDefinition(SAnimationDesc* desc, const CSkeletonInfo** skeleton, string* dbaPath,
    const SAnimationDefinition& definition, CAnimationConvertor* convertor, const char* fileName, const char* sourceGameFolderPath)
{
...
bool CAnimationCompiler::ProcessCBA()
{
    ...
    if (!bSingleFile)
    {
        ...
        if (CheckIfRefreshNeeded(m_CC.GetSourcePath(), GetOutputPath(), &srcFileTimestamp))
        {
            ...
            bRefresh = true;
            ::CopyFile(m_CC.GetSourcePath(), GetOutputPath(), FALSE);
        ...
        for (size_t i = 0; i < numDefinitions; ++i)
        {
            ...
            for (size_t anim = 0; anim < numFiles; ++anim)
            {
                ...
                if (!GetFromDefinition(&animDesc, &skeleton, &dbaPath,
                        *definition, Convertor(), fileName.c_str(), Convertor()->GetSourceGameFolderPath()))
                {
...
bool CAnimationCompiler::Process()
{
            ...
            Convertor()->m_skeletonManager.get()))
    {
        ...
        if (!definition ||
            !GetFromDefinition(&animDesc, &skeleton, &dbaPath, *definition, Convertor(), fileName.c_str(), sourceGameFolderPath.c_str()))
        {
            if (!bErrorReported)
            {
    ...
    if (isAIM || isLook)
    {
        ...
        animDesc.oldFmt.m_fPOS_EPSILON = 0;
        animDesc.oldFmt.m_fROT_EPSILON = 0;
...
static void ProcessAnimationJob(AnimationJob* job)
{
    ...
    if (job->m_singleFileMode)
    {
        ...
        if (!compressor.LoadCAF(job->m_sourcePath.c_str(), *platform, *animDesc, eCAFLoadUncompressedOnly))
        {
    ...
    else
    {
        ...
        if (context.pRC->GetVerbosityLevel() >= 1)
        {
            ...
            if (fileChanged || failedToLoadCompressed)
            {
                _snprintf_s(processTime, sizeof(processTime), _TRUNCATE, "in %.1f sec", float(durationMS) * 0.001f);
            }
            else
            {
        ...
        if ((hFile = _findfirst64(fullPath.c_str(), &c_file)) != -1L)
        {
            ...
            do
            {
                if (!(c_file.attrib & _A_SUBDIR))
                {
                    ...
                    if (pos != string::npos && (_stricmp(".caf", ext.Right(strlen(ext) -  pos)) == 0 || _stricmp(".i_caf", ext.Right(strlen(ext) -  pos))))
                    {
                        ...
                        {
                            if (StringHelpers::StartsWithIgnoreCase(filename, dirsToIgnore[i]))
                            {
                                skip = true;
                                break;
                            }
            ...
            do
            {
                if (c_file.attrib & _A_SUBDIR)
                {
                    ...
                    if (c_file.name[0] != '.')
                    {
                        ...
                        newPath += c_file.name;
                        newPath += "\\";
                        subDirsToScan.push_back(newPath);
                    }
                }
            }   while (_findnext64(hFile, &c_file) == 0);
            _findclose(hFile);

            for (size_t i = 0; i < subDirsToScan.size(); ++i)
            {
                if (ScanDirectoryRecursive(root, subDirsToScan[i], file, files, recursive, dirsToIgnore, sizes))
                {
                    anyFound = true;
                }
            }
