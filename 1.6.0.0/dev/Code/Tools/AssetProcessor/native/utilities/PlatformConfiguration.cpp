...
void PlatformConfiguration::ReadRecognizersFromConfigFile(QString iniPath)
{
    QDir engineRoot;
    ...
    if (QFile::exists(iniPath))
    {
        ...
        for (QString group : groups)
        {
            if (group.startsWith("ScanFolder ", Qt::CaseInsensitive))
            {
                ...
                {
                    AddScanFolder(ScanFolderInfo(watchFolder, outputPrefix, isRoot, recurse, order));
                }
            ...
            if (group.startsWith("RC ", Qt::CaseInsensitive))
            {
                ...
                ReadRecognizerFromConfig(rec, loader);
...
bool PlatformConfiguration::ConvertToRelativePath(QString fullFileName, QString& relativeName, QString& scanFolderName) const
{
    ...
    if (info)
    {
        ...
        if (info->OutputPrefix().isEmpty())
        {
            relativeName = relPath;
        }
        else
        {
            relativeName = info->OutputPrefix();
...
QString PlatformConfiguration::GetOverridingFile(QString relativeName, QString scanFolderName) const
{
    for (int pathIdx = 0; pathIdx < m_scanFolders.size(); ++pathIdx)
    {
        ...
        if (!scanFolderInfo.OutputPrefix().isEmpty() && tempRelativeName.startsWith(scanFolderInfo.OutputPrefix(), Qt::CaseInsensitive))
        {
            tempRelativeName = tempRelativeName.right(tempRelativeName.length() - scanFolderInfo.OutputPrefix().length() - 1); // also eat the slash, hence -1
        }
...
int PlatformConfiguration::ReadGems(QString gemsConfigName)
{
    QDir engineRoot;
    AssetUtilities::ComputeEngineRoot(engineRoot);
    QString normalizedRoot = AssetUtilities::NormalizeDirectoryPath(engineRoot.absolutePath());
