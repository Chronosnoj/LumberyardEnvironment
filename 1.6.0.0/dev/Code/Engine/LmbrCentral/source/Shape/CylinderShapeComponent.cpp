...
namespace LmbrCentral
{
    ...
    AZ_SCRIPTABLE_EBUS(
        ...
        CylinderShapeComponentRequestsBus,
        "{CA00FDE0-A829-4D64-9993-C206BA2ECE7F}",
        "{90AE87F8-22B4-41DB-BCF8-C10CFF2C989F}",
        AZ_SCRIPTABLE_EBUS_EVENT_RESULT(CylinderShapeConfiguration, CylinderShapeConfiguration(), GetCylinderConfiguration)
        AZ_SCRIPTABLE_EBUS_EVENT(SetHeight, float)
        AZ_SCRIPTABLE_EBUS_EVENT(SetRadius, float)
        )
    ...
    void CylinderShapeComponent::Reflect(AZ::ReflectContext* context)
    {
        ...
        if (script)
        {
            ScriptableEBus_CylinderShapeComponentRequestsBus::Reflect(script);
        }
    }
    ...
    void CylinderShapeComponent::SetHeight(float newHeight)
    {
        ...
        m_intersectionDataCache.SetCacheStatus(CylinderShapeComponent::CylinderIntersectionDataCache::CacheStatus::Obsolete_ShapeChange);
        EBUS_EVENT_ID(GetEntityId(), ShapeComponentNotificationsBus, OnShapeChanged);
    }
