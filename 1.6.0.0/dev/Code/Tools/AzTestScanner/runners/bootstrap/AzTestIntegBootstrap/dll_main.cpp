...
int Initialize()
{
    ...
    // on windows pc, we also might have access to the asset cache directly, in which case, go look there.
    ...
    string originalGameFolder = engineCfg.m_gameFolder;

    // do we have access to the asset cache?
    char checkPath[AZ_MAX_PATH_LEN] = { 0 };

    sprintf_s(checkPath, "%s/cache/%s/%s/engineroot.txt", originalRoot.c_str(), originalGameFolder.c_str(), engineCfg.m_assetPlatform);
    if (GetFileAttributesA(checkPath) != INVALID_FILE_ATTRIBUTES)
    {
        // alter it to be the asset cache instead (PC-specific)
        sprintf_s(startupParams.rootPath, "%s/cache/%s/%s", originalRoot.c_str(), originalGameFolder.c_str(), engineCfg.m_assetPlatform);
