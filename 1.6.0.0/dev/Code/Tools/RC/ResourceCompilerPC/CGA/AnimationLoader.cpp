...
bool CAnimationCompressor::ProcessAnimation(bool debugCompression)
{
    ...
    ReplaceRootByLocator(true, minRootPosEpsilon, minRootRotEpsilon);   //replace root by locator, apply simple compression to the root-joint and re-transform all children
    ReplaceRootByLocator(false, minRootPosEpsilon, minRootRotEpsilon);  //apply simple compression to the root-joint and re-transform all children
...
void CAnimationCompressor::ReplaceRootByLocator(bool bCheckLocator, float minRootPosEpsilon, float minRootRotEpsilon)
{
    uint32 numJoints = m_skeleton.m_SkinningInfo.m_arrBonesDesc.size();
    if (numJoints == 0)
    {
