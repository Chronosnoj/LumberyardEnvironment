...
namespace LmbrCentral
{
    ...
    void EditorConstraintComponent::Reflect(AZ::ReflectContext* context)
    {
        ...
        if (serializeContext)
        {
            ...
            if (editContext)
            {
                editContext->Class<EditorConstraintComponent>("Constraint", "The constraint component facilitates constraining two physicalized entities to each other, or a single entity to a point in world space.")
                    ->ClassElement(AZ::Edit::ClassElements::EditorData, "")
