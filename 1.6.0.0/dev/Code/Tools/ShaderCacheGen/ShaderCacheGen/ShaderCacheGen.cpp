...
int main(int argc, char* argv[])
{
    ...
    // alter root to be the asset cache instead (PC-specific)
    sprintf_s(sip.rootPath, "%s/cache/%s/%s", cfg.m_rootFolder.c_str(), cfg.m_gameFolder.c_str(), cfg.m_assetPlatform.c_str());
    sprintf_s(sip.assetsPath, "%s/%s", sip.rootPath, cfg.m_gameFolder.c_str());
