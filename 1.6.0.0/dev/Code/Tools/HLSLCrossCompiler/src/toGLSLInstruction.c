...
void SetDataTypes(HLSLCrossCompilerContext* psContext, Instruction* psInst, const int32_t i32InstCount, SHADER_VARIABLE_TYPE* aeCommonTempVecType)
{
    ...
    for (i = 0; i < i32InstCount; ++i, psInst++)
    {
        ...
        case OPCODE_ISHR:
        {
            ...
            {
                if (GetOperandDataType(psContext, &psInst->asOperands[k]) == SVT_INT ||
                    psInst->asOperands[k].eModifier == OPERAND_MODIFIER_NEG ||
                    psInst->asOperands[k].eModifier == OPERAND_MODIFIER_ABSNEG)
                {
                    eNewType = SVT_INT;
                    break;
                }
