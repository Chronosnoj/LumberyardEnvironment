...
namespace CodeGenerator
{
    ...
    size_t Parser::ParseAttribute(const std::string& str, AttributeList& list)
    {
        ...
        size_t stringCount = 0; // Used to support strings
        bool escape = false;

        // Stop processing if we find a paren, unless we're currently looking in a string
        while (*text && ( ( stringCount && ( stringCount % 2 ) ) || (*text != '(' && *text != ')')))
        {
            ...
            if (stringCount % 2 == 0 && (*text == ' ' || *text == ',' || *text == '\t'))
            {
                break;
            }

            // Detect escape sequences
            if ((stringCount && (stringCount % 2)))
            {
                if (!escape && *text == '\\')
                {
                    escape = true;
