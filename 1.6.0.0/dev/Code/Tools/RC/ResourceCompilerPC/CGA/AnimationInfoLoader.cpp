...
bool CAnimationInfoLoader::LoadDescription(const string& name, IPakSystem* pPakSystem)
{
    ...
    for (uint32 nDefinitionNode = 0; nDefinitionNode < numDefinitions; ++nDefinitionNode)
    {
...

        {
            ...
            if (compression == -1 || !pRoot->getChild(compression)->getAttr("value", def.m_MainDesc.oldFmt.m_CompressionQuality))
            {
...

        {
            ...
            {
                uint32 status = pRoot->getChild(ROTEpsilon)->getAttr("value", def.m_MainDesc.oldFmt.m_fROT_EPSILON);
                if (status == 0)
                {
        ...
        }
        {
            ...
            {
                uint32 status = pRoot->getChild(POSEpsilon)->getAttr("value", def.m_MainDesc.oldFmt.m_fPOS_EPSILON);
                if (status == 0)
                {
                    RCLog("AnimationInfoLoader: ROT_EPSILON value not found in \"%s\" node \"%s\"", pRoot->getTag(), def.m_Model.c_str());
                }
        ...
        if (details != -1)
        {
            ...
            for (uint32 nAnims = 0; nAnims < numAnims; ++nAnims)
            {
                ...
                if (!val.empty())
                {
                    pAnim->getAttr("PosEpsilon", desc.oldFmt.m_fPOS_EPSILON);
                }

                val = pAnim->getAttr("RotEpsilon");
                if (!val.empty())
                {
                    pAnim->getAttr("RotEpsilon", desc.oldFmt.m_fROT_EPSILON);
                }

                val = pAnim->getAttr("compression");
                if (!val.empty())
                {
                    pAnim->getAttr("compression", desc.oldFmt.m_CompressionQuality);
                }
