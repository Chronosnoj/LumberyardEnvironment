...
def to_nodes(self, lst, path=None):
    ...
    for x in Utils.to_list(lst):
        ...
        if not node:
            error_detected = True
            Logs.error("(%s): source not found: %r in %r (%r)" % (self.bld.cmd, x, self.target, os.path.normpath(self.path.abspath() + '/wscript')))
        else:
            tmp.append(node)
    if error_detected:
        raise Errors.WafError("One or more source files missing")
    return tmp
