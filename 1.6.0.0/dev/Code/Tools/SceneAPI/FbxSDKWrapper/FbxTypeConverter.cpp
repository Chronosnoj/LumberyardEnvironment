...
namespace AZ
{
    namespace FbxSDKWrapper
    {
        ...
        {
            return Vector2(static_cast<AZ::VectorFloat>(static_cast<float>(vector[0])),
                static_cast<AZ::VectorFloat>(static_cast<float>(vector[1])));
        }
        ...
        Vector3 FbxTypeConverter::ToVector3(const FbxVector4& vector)
        {
            ...
            return Vector3(static_cast<AZ::VectorFloat>(static_cast<float>(vector[0])), 
                static_cast<AZ::VectorFloat>(static_cast<float>(vector[1])),
        ...
        Transform FbxTypeConverter::ToTransform(const FbxAMatrix& matrix)
        {
            ...
            for (int row = 0; row < 3; ++row)
            {
                ...
                {
                    // Note: FbxVector4[x] is of type FbxDouble and aznumeric_caster does not accept it as cast from type.
                    transform.SetElement(row, column, static_cast<AZ::VectorFloat>(static_cast<float>(line[column])));
                }
            }
            return transform;
        }
