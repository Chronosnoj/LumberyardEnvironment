...
static string getTextMatrix(const Matrix34& M)
{
    ...
    _snprintf_s(buff, sizeof(buff), _TRUNCATE, "axisX(%g %g %g)", M.m00, M.m10, M.m20);
    text += buff;
    _snprintf_s(buff, sizeof(buff), _TRUNCATE, " axisY(%g %g %g)", M.m01, M.m11, M.m21);
    text += buff;
    _snprintf_s(buff, sizeof(buff), _TRUNCATE, " axisZ(%g %g %g)", M.m02, M.m12, M.m22);
    text += buff;
    _snprintf_s(buff, sizeof(buff), _TRUNCATE, " trans(%g %g %g)", M.m03, M.m13, M.m23);
    text += buff;
...
static string getTextPhysicalizeType(int const type)
{
    ...
    default:
    {
        text += "*** UNKNOWN ***";
        char buff[50];
        _snprintf_s(buff, sizeof(buff), _TRUNCATE, " (0x%x)", type);
        text += buff;
    }
    break;
    }
...
static string getTextPhysicalizeFlags(int const flags)
{
    ...
    default:
    {
        text += "*** UNKNOWN ***";
        char buff[50];
        _snprintf_s(buff, sizeof(buff), _TRUNCATE, " (0x%x)", flags);
        text += buff;
    }
    break;
    }
