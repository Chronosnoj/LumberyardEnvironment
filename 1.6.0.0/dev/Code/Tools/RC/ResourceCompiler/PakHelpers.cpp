...
bool PakHelpers::PakEntry::MakeSortableStreamingSuffix(const string& suffix, string* sortable, int nDigits, int nIncrement)
{
    ...
    case 2:
        // .dds.0a, .dds.1a, etc.
        _snprintf_s(verify, sizeof(verify), _TRUNCATE, "%d%c", nMipmap, cAttached);
        break;
    case 1:
        // .dds.0, .dds.1, etc.
        _snprintf_s(verify, sizeof(verify), _TRUNCATE, "%d", nMipmap);
        break;
    default:
        // .dds.a
        if (sscanf(suffix.data(), "%c", &cAttached) == 1)
        {
            _snprintf_s(verify, sizeof(verify), _TRUNCATE, "%c", cAttached);
            nTokens = 2;
        }
        break;
    }
    ...
    if (sortable)
    {
        ...
        if (nDigits > 0)
        {
            _snprintf_s(digits, sizeof(digits), _TRUNCATE, "%%0%dd", nDigits);
        }
        else
        {
            cry_strcpy(digits, "%d");
        ...
        {
        case 2:
            _snprintf_s(verify, sizeof(verify), _TRUNCATE, digits, nMipmap + nIncrement, cAttached);
            break;
        case 1:
            _snprintf_s(verify, sizeof(verify), _TRUNCATE, digits, nMipmap + nIncrement);
            break;
        default:
            break;
        }
