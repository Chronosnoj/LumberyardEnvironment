...
def run_commands():
	...
	# Disable host tools build for now until we fix azcg and waf problems with not having clang
	should_build_host_tools = False
	if should_build_host_tools:
		# Inject build host tools after configuration/clean but before build commands
		for index, command in enumerate(Options.commands):
			if 'build' in command:
				Options.commands.insert(index, 'build_host_tools')
				break
