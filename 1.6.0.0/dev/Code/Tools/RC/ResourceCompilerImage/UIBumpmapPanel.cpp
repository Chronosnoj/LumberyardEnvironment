...
#include <windows.h>              // GWLP_USERDATA
...
void CUIBumpmapPanel::InitDialog(HWND hWndParent, const bool bShowBumpmapFileName)
{
...

    {
        ...
        HWND hwnd = GetDlgItem(m_hTab_Normalmapgen, IDC_SRFO_ADDBUMP_TEXT0);
        assert(hwnd);

        ShowWindow(hwnd, nCmdShow);

        hwnd = GetDlgItem(m_hTab_Normalmapgen, IDC_SRFO_ADDBUMP_NAME);
        assert(hwnd);

        ShowWindow(hwnd, nCmdShow);

        hwnd = GetDlgItem(m_hTab_Normalmapgen, IDC_SRFO_ADDBUMP_CHOOSE);
        assert(hwnd);
    ...
    {
        hwnd = GetDlgItem(m_hTab_Normalmapgen, IDC_NM_SLIDERBUMPSTRENGTH);
        assert(hwnd);
    ...
    {
        hwnd = GetDlgItem(m_hTab_Normalmapgen, IDC_NM_SLIDERBUMPBLUR);
        assert(hwnd);
    ...
    {
        hwnd = GetDlgItem(m_hTab_Normalmapgen, IDC_EDIT_BUMPBLUR);
        assert(hwnd);
    ...
    {
        hwnd = GetDlgItem(m_hTab_Normalmapgen, IDC_EDIT_BUMPSTRENGTH);
        assert(hwnd);
    ...
    {
        hwnd = GetDlgItem(m_hTab_Normalmapgen, IDC_NM_BUMPINVERT);
        assert(hwnd);
