...
@feature('cxxshlib', 'cshlib', 'cxx', 'c')
@after_method('apply_incpaths')
@after_method('process_pch_msvc')
def add_import_lib_for_secondary_install(self): 
    # only add the import library to the output list (allowing it to be copied) if we're trying to do an install 
    if getattr(self, 'link_task', None) and self._type == 'shlib' and getattr(self, 'is_secondary_copy_install', False):
        importlibnode = self.link_task.outputs[0].change_ext('.lib')
        self.link_task.outputs.append(importlibnode)  
...
def set_pdb_flags(self):            
    ...
    for t in self.tasks:
        if isinstance(t,pch_msvc):
            ...
            t.env.append_value('CXXFLAGS',rtti_flag)
...
def wrap_class_orbis(class_name):
    ...
    def exec_command(self, *k, **kw):
        # ToDo: Move response file usage to somewhere other than orbis specific functions in a file called msvc_helper !
        if self.env['CC_NAME'] in ('orbis-clang', 'gcc', 'clang'):
