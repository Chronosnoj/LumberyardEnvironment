...
EPreviewMode CImageUserDialog::GetPreviewMode(const CImageProperties& props, HWND control)
{
    ...
    else if (!StringHelpers::Equals(s_pComboboxSeparator, text))
    {
        for (int i = 0; i < ePM_Num; ++i)
        {
            ...
            if (!_stricmp(text, CImagePreview::GetPreviewModeChoiceText(pm)))
            {
                return pm;
            }
        }
...
void CImageUserDialog::SetDataToDialog(const bool bInitalUpdate)
{
    ...
    hwnd = GetDlgItem(m_hWindow, IDC_PREVIEWON);
    assert(hwnd);
    ...
    hwnd = GetDlgItem(m_hTab_Resolution, IDC_SUPRESSENGINEREDUCE);
    assert(hwnd);
...
void CImageUserDialog::GetDataFromDialog()
{
    ...
    hwnd = GetDlgItem(m_hWindow, IDC_PREVIEWMODE);
    assert(hwnd);
...
INT_PTR CALLBACK CImageUserDialog::WndProc_ChooseResolution(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        {
            char str[128];
            _snprintf_s(str, sizeof(str), _TRUNCATE, "%i", i);
            ...
            _snprintf_s(str, sizeof(str), _TRUNCATE, "%u x %u  (%i)", dwWidth, dwHeight, i);

            int id;
            switch (i - Reduce::kMinReduce)
            {
            case 0:
                id = IDC_CHOOSERESOLUTION_BTN0;
                break;
            case 1:
                id = IDC_CHOOSERESOLUTION_BTN1;
                break;
            case 2:
                id = IDC_CHOOSERESOLUTION_BTN2;
                break;
            case 3:
                id = IDC_CHOOSERESOLUTION_BTN3;
                break;
            case 4:
                id = IDC_CHOOSERESOLUTION_BTN4;
                break;
            case 5:
                id = IDC_CHOOSERESOLUTION_BTN5;
                break;
            case 6:
                id = IDC_CHOOSERESOLUTION_BTN6;
                break;
            default:
                id = IDC_CHOOSERESOLUTION_BTN7;
                break;
            }
    ...
    case WM_COMMAND:
    {
        ...
        {
        case IDC_CHOOSERESOLUTION_BTN0:
            idx = 0;
            break;
        case IDC_CHOOSERESOLUTION_BTN1:
            idx = 1;
            break;
        case IDC_CHOOSERESOLUTION_BTN2:
            idx = 2;
            break;
        case IDC_CHOOSERESOLUTION_BTN3:
            idx = 3;
            break;
        case IDC_CHOOSERESOLUTION_BTN4:
            idx = 4;
            break;
        case IDC_CHOOSERESOLUTION_BTN5:
            idx = 5;
            break;
        case IDC_CHOOSERESOLUTION_BTN6:
            idx = 6;
            break;
        case IDC_CHOOSERESOLUTION_BTN7:
            idx = 7;
            break;
        case IDCANCEL:
            idx = -1;
...
{
    CImageUserDialog* This = (CImageUserDialog*)(LONG_PTR)GetWindowLongPtr(hWnd, GWLP_USERDATA);
...
DWORD CImageUserDialog::ThreadStart()
{
    ...
    // update preview and warning subtitles
    {
        ...
        assert(hwnd2);
        HWND hwndW = GetDlgItem(m_hWindow, IDC_WARNINGLINE);
        assert(hwndW);
