...
public:
    ...
    void GetMemoryUsage(ICrySizer* pSizer) const { assert(0); };
    ...
    void shareChildren(const XmlNodeRef& fromNode) { assert(0); };

    // Summary:
    //   Returns XML of this node and sub nodes into tmpBuffer without XML checks (much faster)
    XmlString getXMLUnsafe(int level, char* tmpBuffer, uint32 sizeOfTmpBuffer) const { return getXML(level); }

    // Notes:
    //   Save in small memory chunks.
    bool saveToFile(const char* fileName, size_t chunkSizeBytes, FILE* file = NULL) { assert(0); return false; };
    // </interfuscator:shuffle>
#endif
