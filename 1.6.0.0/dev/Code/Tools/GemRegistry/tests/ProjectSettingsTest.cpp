...
TEST_F(ProjectSettingsTest, ParseTest)
{
    ...
    ASSERT_TRUE(ps.ParseJson(json))                 << m_errDescriptionParseFailed << m_registry->GetErrorMessage().c_str();
    auto gemMap = ps.GetGems();
    EXPECT_EQ(gemMap.size(), 1)                     << m_errDescriptionParseFailed << m_registry->GetErrorMessage().c_str();
    EXPECT_EQ(gemMap.begin()->second.m_id, id)      << m_errDescriptionParseFailed << m_registry->GetErrorMessage().c_str();
    EXPECT_EQ(gemMap.begin()->second.m_version, v1) << m_errDescriptionParseFailed << m_registry->GetErrorMessage().c_str();
