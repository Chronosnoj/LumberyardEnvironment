...
def use_incredibuild_durango(ctx, section_name, option_name, value):
    """ If Incredibuild should be used, check for required packages """
    return internal_use_incredibuild(ctx, section_name, option_name, value, ATTRIBUTE_VERIFICATION_CALLBACKS['verify_use_incredibuild_durango'])
...
def verify_use_incredibuild_durango(ctx, option_name, value):
    ...
    if not _is_user_option_true(value):
        return (True,"","")
    (res, warning, error) = internal_verify_incredibuild_license('IncrediBuild for Xbox One', 'Durango')
    return (res, warning, error)
...
def use_incredibuild_orbis(ctx, section_name, option_name, value):
    """ If Incredibuild should be used, check for required packages """
    return internal_use_incredibuild(ctx, section_name, option_name, value, ATTRIBUTE_VERIFICATION_CALLBACKS['verify_use_incredibuild_orbis'])
...
def verify_use_incredibuild_orbis(ctx, option_name, value):
    ...
    if not _is_user_option_true(value):
        return (True,"","")
    (res, warning, error) = internal_verify_incredibuild_license('IncrediBuild for PlayStation', 'Orbis')
    return (res, warning, error)
...
@register_attribute_callback    
def out_folder_win64(ctx, section_name, option_name, value):
    """ Configure output folder for win x64 """
    if not _is_user_input_allowed(ctx, option_name, value):
        Logs.info('\nUser Input disabled.\nUsing default value "%s" for option: ""%s"' % (value, option_name))
        return value
    ...
    _output_folder_disclaimer(ctx)
    return _get_string_value(ctx, 'Win x64 Output Folder', value)   
