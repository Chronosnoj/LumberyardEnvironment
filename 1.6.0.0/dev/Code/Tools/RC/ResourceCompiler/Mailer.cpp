...
void CSMTPMailer::ReceiveLine(SOCKET connection)
{
    ...
    if (ret == SOCKET_ERROR)
    {
        _snprintf_s(buf, sizeof(buf), sizeof(buf) - 1, "Error: WinSock error %d during recv()\n", WSAGetLastError());
    }
    else
    {
        buf[ret] = 0;
...
SOCKET CSMTPMailer::Open(const char* host, unsigned short port, sockaddr_in& serverAddress)
{
    ...
    {
        char buf[1025];
        _snprintf_s(buf, sizeof(buf), sizeof(buf) - 1, "Error: Host %s not found\n", host);
        m_response += buf;
...
bool CSMTPMailer::Send(const tstr& from, const tstrcol& to, const tstrcol& cc, const tstrcol& bcc, const tstr& subject, const tstr& body, const tstrcol& attachments)
{
    ...
    {
        char buf[1025];
        _snprintf_s(buf, sizeof(buf), sizeof(buf) - 1, "Error: Failed to connect to %s:%d\n", m_server.c_str(), m_port);
        m_response += buf;
        return false;
    }
