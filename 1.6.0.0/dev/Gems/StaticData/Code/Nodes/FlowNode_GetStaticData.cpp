...
namespace LmbrAWS
{
    ...
    FlowNode_GetStaticData::FlowNode_GetStaticData(SActivationInfo* activationInfo) : BaseMaglevFlowNode< eNCT_Instanced >(activationInfo)
    {        
    ...
    void FlowNode_GetStaticData::DoNodeActivation(SActivationInfo* activationInfo)
    {
        ...
        const auto& dataType = Aws::String{ GetPortString(activationInfo, EIP_StaticDataType).c_str() };
        const auto& dataId = Aws::String{ GetPortString(activationInfo, EIP_StaticDataId).c_str() };
        const auto& fieldName = Aws::String{ GetPortString(activationInfo, EIP_FieldName).c_str() };
        ...
        bool gotStr = false;
        EBUS_EVENT_RESULT(gotStr, CloudCanvas::StaticData::StaticDataRequestBus,
            GetStrValue, dataType.c_str(), dataId.c_str(), fieldName.c_str(), strValue);
        if (gotStr)
        {
        ...
        bool gotInt = false;
        EBUS_EVENT_RESULT(gotInt, CloudCanvas::StaticData::StaticDataRequestBus,
            GetIntValue, dataType.c_str(), dataId.c_str(), fieldName.c_str(), numValue);
        if (gotInt)
        {
            SFlowAddress addr(activationInfo->myID, EOP_NumberOut, true);
            activationInfo->pGraph->ActivatePort(addr, numValue);
            SuccessNotify(activationInfo->pGraph, activationInfo->myID);
            gotSuccess = true;
        }

        bool gotDouble = false;
        EBUS_EVENT_RESULT(gotDouble, CloudCanvas::StaticData::StaticDataRequestBus,
            GetDoubleValue, dataType.c_str(), dataId.c_str(), fieldName.c_str(), doubleValue);
        if (gotDouble)
        {
            SFlowAddress addr(activationInfo->myID, EOP_FloatOut, true);
            activationInfo->pGraph->ActivatePort(addr, static_cast<float>(doubleValue));
            SuccessNotify(activationInfo->pGraph, activationInfo->myID);
            gotSuccess = true;
        }
