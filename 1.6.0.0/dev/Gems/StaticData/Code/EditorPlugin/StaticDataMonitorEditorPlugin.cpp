...
void StaticDataMonitorEditorPlugin::OnEditorNotify(EEditorNotifyEvent aEventId)
{
    ...
    case eNotify_OnInit:
    {
        CloudCanvas::StaticData::IStaticDataManager* pManager = CloudCanvas::StaticData::StaticDataRequestBus::FindFirstHandler();
        if (pManager)
        {
            if (m_monitor)
            {
                m_monitor->SetManager(pManager);
            }
            pManager->ReloadTagType("");
        }
    }
    break;
    case eNotify_OnQuit:
