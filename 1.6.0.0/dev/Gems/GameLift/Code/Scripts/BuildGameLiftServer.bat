...
:CompileAssets
	...
	CALL bin64\AssetProcessorBatch.exe /gamefolder=%PROJECT_NAME% /platforms=pc || GOTO :AssetProcessingFailed

	ECHO ----- CREATING PACKAGES ----
	CALL bin64\rc\rc.exe /job=bin64\rc\RCJob_Generic_MakePaks.xml /p=pc /game=%PROJECT_NAME% /trg=%PROJECT_NAME%_pc_Paks_Dedicated || GOTO :RCFailed
	...
	XCOPY %DESTINATION_PROJECT_FOLDER_NAME%\..\Bin64.Release.Dedicated\* %CD%\Bin64.Release.Dedicated\ /Y /I /F /R /EXCLUDE:exclusion_list.txt || (DEL exclusion_list.txt && GOTO :CopyFailed)
	DEL exclusion_list.txt
	
	ECHO ----- COPYING ADDITIONAL FILES -----
	XCOPY "%DESTINATION_PROJECT_FOLDER_NAME%\..\Bin64\Redistributables\Visual Studio 2013\vcredist_x64.exe" %CD% /Y /I /F /R || GOTO :CopyFailed
